...
export default function FlavorStory({ id, heading, body, image, imageAlt, flipped = false, imageScale, textGap, compact, imageOffset, imageZoom }: FlavorStoryProps) {
  return (
    <section id={id} className={`relative px-6 overflow-hidden ${compact ? "pt-16 pb-6 md:pt-20 md:pb-12" : "py-16 md:py-24"}`}>
      <div className={`max-w-6xl mx-auto relative ${compact ? "min-h-[500px] sm:min-h-[600px] md:min-h-[700px]" : "min-h-[600px] sm:min-h-[700px] md:min-h-[800px]"}`}>
        ...
        <Draggable id={`${id}-image`} className={`absolute top-0 ${flipped ? "left-0" : "right-0"} h-full ${imageScale || "w-[70%] md:w-[65%]"}`}>
          <motion.div
            initial={{ opacity: 0, x: flipped ? -80 : 80 }}
            whileInView={{ opacity: 1, x: 0 }}
